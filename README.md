...
**Data Cleaning**
	In the raw dataset, some movies had a missing budget and revenue. Many of the movies have a null runtime. All these features are necessary for analysis, so the movie is dropped if any of these features are missing. 
  
  A movie is also dropped if it is a pornographic film. This is because adult movies are not eligable for an Oscar.
...
•	release_date – replaced by year
•	status – dataset assumes all movies are released
•	genres – replaced by single genre
•	production_countries – replaced by single country
•	production_companies – too much variance in companies
